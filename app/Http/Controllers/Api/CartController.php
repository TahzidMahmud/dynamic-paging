...
class CartController extends Controller
{
    ...
    public function add(Request $request)
    {
        ...
        $product_variation = ProductVariation::with(['product.shop','combinations.attribute','combinations.attribute_value'])->findOrFail($validated['variation_id']);
        ...
        $cart = Cart::updateOrCreate([
                ...
                'quantity' => DB::raw('quantity + '.$validated['qty'])
            ]);

        $product = [
            'cart_id' => (integer) $cart->id,
            'unit'=>$product_variation->product->unit,
