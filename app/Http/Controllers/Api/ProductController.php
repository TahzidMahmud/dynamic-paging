...
use App\Http\Resources\SkinTypeCollection;
use App\Http\Resources\ConcernCollection;
...
class ProductController extends Controller
{
    ...
    public function featured(){
        $featured_products =  Product::where('featured', 1)->where('published', 1)->latest()->take(10)->get();
        return new ProductCollection($featured_products);
    }
    public function popular(){
        $popular_products =  Product::where('published', 1)->orderBy('num_of_sale','desc')->take(10)->get();
        return new ProductCollection($popular_products);
    }
    public function search(Request $request)
    {
        ...
        $brand                      = $request->brand_slug ? Brand::where('slug', $request->brand_slug)->first() : null;
        $brand_ids                  = $request->brand_ids ? explode(',',$request->brand_ids) : null;
        $skin_type_ids              = $request->skin_type_ids ? explode(',',$request->skin_type_ids) : null;
        $concern_ids                = $request->concern_ids ? explode(',',$request->concern_ids) : null;
        ...
        if($skin_type_ids != null){
            $product_ids = ProductSkinType::whereIn('skin_type_id', $skin_type_ids)->pluck('product_id')->toArray();
            if($product_ids != null){
                $products->whereIn('id', $product_ids);
            }
        }
        if($concern_ids != null){
            $product_ids = ProductConcern::whereIn('concern_id', $concern_ids)->pluck('product_id')->toArray();
            if($product_ids != null){
                $products->whereIn('id', $product_ids);
            }
        }
        ...
        if ($selected_attribute_values != null) {
        ...
        return response()->json([
            ...
            'skinTypes'=>new SkinTypeCollection(SkinType::all()),
            'concerns'=>new ConcernCollection(Concern::all()),
        ]);
    }
