        ...
        <table class="table aiz-table mb-0">
            <thead>
                <tr>
                    ...
                    <th>{{translate('Flat Delivery Cost')}}</th>
            ...
            <tbody>
                @foreach($zones as $key => $zone)
                    <tr>
                        ...
                        <td>
                            @foreach (json_decode($zone->states ?? '[]') as $state_id)
                                @php $state = App\Models\State::find($state_id); @endphp
                                @if($state)
                                    <span class="badge badge-inline badge-md bg-soft-dark mb-1">{{ $state->name }}</span>
                                @endif
                            @endforeach
                        </td>
                        <td>
                            {{ format_price($zone->flat_delivery_cost) }}
						</td>
                        <td class="text-right">
