...
@section('content')
                    ...
                    <form class="form-horizontal" action="{{ route('settings.update') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                ...
                                <div class="aiz-radio-list">
                                    <label class="aiz-radio">
                                        <input type="radio" name="active_sms_gateway" value="bulksmsbd" @if(get_setting('active_sms_gateway') == 'bulksmsbd') checked @endif>
                                        <span class="aiz-rounded-check"></span>
                                        <span>{{ translate('bulksmsbd') }}</span>
                                    </label>
                                    <label class="aiz-radio">
    ...
    <div class="row">
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0 h6 ">{{ translate('Admin Number for SMS Update') }}</h5>
                </div>
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('settings.update') }}" method="POST">
                        @csrf
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="sms_update_number_admin">
                            <div class="col-lg-3">
                                <label class="col-from-label">{{translate('Admin Number')}}</label>
                            </div>
                            <div class="col-lg-6">
                                <input type="text" class="form-control" name="sms_update_number_admin" value="{{ get_setting('sms_update_number_admin') }}" placeholder="Number" required>
                            </div>
                        </div>
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0 h6 ">BULKSMSBD Credential</h5>
                </div>
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('env_key_update.update') }}" method="POST">
                        @csrf
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="BULK_SMS_SENDER_ID">
                            <div class="col-lg-3">
                                <label class="col-from-label">{{translate('BULKSMS SENDER ID')}}</label>
                            </div>
                            <div class="col-lg-6">
                                <input type="text" class="form-control" name="BULK_SMS_SENDER_ID" value="{{  env('BULK_SMS_SENDER_ID') }}" placeholder="bulksmsbd sender id" required>
                            </div>
                        </div>
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="BULK_SMS_API_KEY">
                            <div class="col-lg-3">
                                <label class="col-from-label">{{translate('BULK_SMS_API_KEY')}}</label>
                            </div>
                            <div class="col-lg-6">
                                <input type="text" class="form-control" name="BULK_SMS_API_KEY" value="{{  env('BULK_SMS_API_KEY') }}" placeholder="bulksmsbd api key" required>
                            </div>
                        </div>
                        <div class="form-group mb-0 text-right">
                            <button type="submit" class="btn btn-primary">{{ translate('Update') }}</button>
