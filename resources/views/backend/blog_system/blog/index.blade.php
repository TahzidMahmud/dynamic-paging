            ...
            <table class="table mb-0 aiz-table">
                <thead>
                    <tr>
                        ...
                        <th data-breakpoints="lg">{{translate('Home Page')}}</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input type="checkbox" onchange="change_featured(this)" value="{{ $blog->id }}" <?php if($blog->featured == 1) echo "checked";?>>
                                <span></span>
                            </label>
                        </td>
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input type="checkbox" onchange="change_home_featured(this)" value="{{ $blog->id }}" <?php if($blog->home_featured == 1) echo "checked";?>>
                                <span></span>
                            </label>
                        </td>
                        <td class="text-right">
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        function change_featured(el){
            ...
            $.post('{{ route('blog.change-featured') }}', {_token:'{{ csrf_token() }}', id:el.value, featured:featured}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Change blog featured successfully') }}');
                }
                else{
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
        ...
        function change_home_featured(el){
            var home_featured = 0;
            if(el.checked){
                var home_featured = 1;
            }
            $.post('{{ route('blog.change-home-featured') }}', {_token:'{{ csrf_token() }}', id:el.value, home_featured:home_featured}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Change blog home featured successfully') }}');
                }
                else{
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
