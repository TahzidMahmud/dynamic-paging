    ...
    <tbody>
        @foreach ($variations as $key => $variation)
            ...
            <tr class="variant">
                ...
                <td>
                    <input type="number" step="0.01" name="variations[{{ $key }}][purchase_price]"
                        value="{{ $variation->purchase_price }}" min="0" class="form-control" required>
                </td>
                <td>
                    <input type="number" lang="en" min="0" value="{{ $variation->stock }}"
                    placeholder="{{ translate('Stock') }}" name="variations[{{ $key }}][stock]" class="form-control"
                    required>
                </td>
                <td>
                    <input type="text" name="variations[{{ $key }}][sku]" value="{{ $variation->sku }}" class="form-control">
                </td>
                <td>
