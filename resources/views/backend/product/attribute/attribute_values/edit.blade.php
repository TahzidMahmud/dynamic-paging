    ...
    <div class="card">
        <div class="card-body p-0">
			...
			<form class="p-4" action="{{ route('attribute_values.update', $attribute_value->id) }}" method="POST">
                ...
                @if ($attribute_value->attribute->name=='Color' || $attribute_value->attribute->name=='color')
                    <div class="form-group mb-3"> <div class="form-group mb-3">
                        <label for="color_code">{{translate('Give color code ')}}</label>
                        <input type="text" placeholder="{{ translate('color code ')}}" name="color_code" value="{{ $attribute_value->color_code }}" color_code="color_code" class="form-control" required>
                        <div class="alert alert-danger mt-2">
                            {{ translate('Like #ffffff or rgb(255, 0, 0)') }}
                        </div>
                    </div>
                @else
                    <input type="hidden" name="color_code" color_code="color_code" class="form-control" value="">
