    ...
    <div class="row gutters-5">
                    ...
                    <div class="flex-grow-1 row align-items-start">
                        ...
                        <div class="col-md-3 mr-auto ml-0">
                            ...
                            <div class="mb-3">
                                ...
                                @elseif (($order->delivery_status == 'delivered' || $order->delivery_status == 'cancelled') && (Auth::user()->can('change_order_status') || Auth::user()->email == 'admin@example.com'))
                                    <select
                                        class="form-control aiz-selectpicker"
                                        id="update_delivery_status"
                                        data-minimum-results-for-search="Infinity"
                        ...
                        <div class="col-xl-4 col-md-6 ml-auto mr-0">
                            <table class="table">
                                <tbody>
                                    ...
                                    @if ($order->combined_order->advanced_payment!=null)
                                    <tr>
                                        <td>
                                            <strong class=""> {{ translate('Advanced Payment') }} :</strong>

                                        </td>
                                        <td>{{ $order->combined_order->advanced_payment }} (Paid)</td>
                                    </tr>
                                    @endif
                                    <tr>
                                        <td><strong class="">{{ translate('TOTAL') }} :</strong></td>
                                        <td class=" h4">
                                            {{ $order->combined_order->advanced_payment!=null?format_price($order->grand_total- $order->combined_order->advanced_payment):format_price($order->grand_total) }}
                                        </td>
                                    </tr>
        ...
        <div class="col col-lg-auto w-lg-300px">
            ...
            @if ($order->combined_order->advanced_payment!=null)
                <div class="card">
                    <div class="card-header">
                        <h3 class="fs-16 mb-0">{{ translate('Advanded Paymnet Details') }}</h3>
                    </div>
                    <div class="card-body">
                    <h6>Amount Paid: {{ $order->combined_order->advanced_payment}}</h6>
                    <h6>Note:</h6>{{ $order->combined_order->payment_note}}
                    </div>
                </div>
            @endif
            <div class="card">
    ...
    <script type="text/javascript">
        ...
        $('#note-btn').on('click',function(){
            ...
            if(note!=""){
                ...
                $.post('{{ route('orders.update_delivery_status') }}', {_token:'{{ @csrf_token() }}',order_id:order_id,status:status,note:note}, function(data){
                    AIZ.plugins.notify('success', '{{ translate('Delivery status has been updated') }}');
                    $('textarea#note').val('');
                    window.location.reload();
                });
            }else{
                AIZ.plugins.notify('danger', '{{ translate('Must Provide a Note For Status Update') }}');
            }
